...
const Header = () => {
  ...
  return (
    <>
      <div className='container-fluid w-100 d-flex py-2 px-5 justify-content-between position-fixed fixed-top bg-light z-3' ref={container} style={{boxShadow:"0px 10px 10px 1px rgba(146, 146, 146, 0.5)"}}>
          <div className="navbar-brand d-flex">
            <img src={logo} className='navcon' style={{height:"55px"}}></img>
      ...
      <div className="side d-flex align-items-center column-gap-5">
        <div className="btn btn-primary font px-4 rounded-2 navcon">Admission-2025</div>
        <i className="bi bi-list text-light bg-primary fw-bolder  p-1 px-2 fs-4 rounded-circle navcon" onClick={()=>{tl1.current.play()}}></i>
        </div>
        <div className="sidebar position-absolute w-50 bg-light p-5 d-flex flex-column justify-content-between" style={{height:"100vh"}}>
          <div className="close d-flex justify-content-end">
            <i className="bi bi-x-octagon fs-2 pe-5" onClick={() => { tl1.current.reverse(); }}></i>
          </div>
          <ul className="d-flex flex-column nav row-gap-4 align-item-start fw-bold fs-5 ps-3">
          ...
          <div className="bottom ps-3 fw-bold d-flex justify-content-between align-items-center">
